...
struct ParsecStatusBar : View {
	...
	@ObservedObject var model = DataManager.model
	...
	func poll()
	{
		...
		if showMenu
		{
			...
			metricInfo = "Render FPS \(String(format: "%.1f", model.fps))    Video FPS \(String(format: "%.1f", model.videoFps))    Decode \(String(format:"%.2f", pcs.`self`.metrics.0.decodeLatency))ms    Encode \(String(format:"%.2f", pcs.`self`.metrics.0.encodeLatency))ms    Network \(String(format:"%.2f", pcs.`self`.metrics.0.networkLatency))ms    Bitrate \(String(format:"%.2f", pcs.`self`.metrics.0.bitrate))Mbps    \(pcs.decoder.0.h265 ? "H265" : "H264") \(pcs.decoder.0.width)x\(pcs.decoder.0.height) \(pcs.decoder.0.color444 ? "4:4:4" : "4:2:0") \(str)"
		}
	}
}
