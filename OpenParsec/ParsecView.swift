...
struct ParsecStatusBar : View {
	...
	func poll()
	{
		...
		if showMenu
		{
			...
			let fpsStr = String(format: "%.1f", model.videoFps)
			metricInfo = "\(pcs.decoder.0.h265 ? "H265" : "H264") \(pcs.decoder.0.width)x\(pcs.decoder.0.height)/\(fpsStr) fps \(pcs.decoder.0.color444 ? "4:4:4" : "4:2:0") \(str)    Decode \(String(format:"%.2f", pcs.`self`.metrics.0.decodeLatency))ms    Encode \(String(format:"%.2f", pcs.`self`.metrics.0.encodeLatency))ms    Network \(String(format:"%.2f", pcs.`self`.metrics.0.networkLatency))ms    Bitrate \(String(format:"%.2f", pcs.`self`.metrics.0.bitrate))Mbps"
		}
	}
}
